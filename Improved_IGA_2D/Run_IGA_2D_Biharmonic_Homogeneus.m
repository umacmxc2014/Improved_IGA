...
%   test_case = 'plate_hole';
    ...
    %%
t= 0; % Elevate the degree of B-spline basis function to (pu+t) in u-direction, and to (pv+t) in the v-direction
%%
...
Refinement = [7,7,7]; % h-refine the initial mesh 2,3,4,5,6 times.
...
Time=zeros(n_refine,1);
  ...
  for  i=1:n_refine
     tic
 [err(i,:),dof(i)]=IGA_2D_Biharmonic_Homogeneous(ConPts,weights,knotU,pu,knotV,pv,Refinement(i),t, test_case);
     toc
  end
